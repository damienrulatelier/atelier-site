...
export default function CommandePage() {
  ...
  // Statut fidélité (suivi par e-mail, sur l'historique des commandes
  // passées) — utilisé uniquement pour signaler la récompense du 3e achat,
  // pas pour la livraison gratuite qui dépend du panier actuel (voir plus bas).
  useEffect(() => {
  ...
  // Dès que le client tape un email valide sur la page commande, on capture
  // son panier comme "potentiellement abandonné" — si il paye, on marquera
  // ce panier comme converti sur /merci ; sinon, une relance partira 24h après.
  useEffect(() => {
    ...
    }, 2000); // petit délai pour ne pas spammer à chaque frappe
    return () => clearTimeout(timeout);
    ...
    try {
      const res = await fetch("/api/checkout/stripe", {
        ...
        body: JSON.stringify({
          ...
          shippingLabel: freeShipping ? `${shippingOpt.label} (offerte fidélité)` : shippingOpt.label,
  ...
  if (lines.length === 0) {
    return (
      <main className="max-w-2xl mx-auto px-8 py-24 text-center">
        ...
        <a href="/" className="text-[#B23A24] underline text-sm">
    ...
    <main className="max-w-2xl mx-auto px-8 py-14">
      <div className="flex items-center justify-between mb-10">
        ...
        <button
          ...
          className="text-sm text-[#8C8780] hover:text-[#B23A24] transition-colors underline"
        >
          Annuler
        </button>
      </div>
        ...
        <div className="border border-[#DEDAD1] divide-y divide-[#DEDAD1]">
          {lines.map((l) => (
            <div key={l.lineId} className="px-4 py-3 text-sm">
              ...
              {l.isDedicated && (
                <div className="mt-1.5 text-xs italic text-[#3A3631]">✎ {l.dedication}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <section className="mb-9">
        ...
        <div className="grid gap-3.5">
          <input
            ...
            className="border border-[#DEDAD1] px-3 py-2.5 text-sm focus:outline-none focus:border-[#181614]"
          />
          ...
          {loyalty?.freeRewardUnlocked && (
            <div className="bg-[#F2F0EA] border border-[#B23A24] p-4 text-sm text-[#3A3631]">
              🎁 <strong>Tu as débloqué un dessin offert !</strong> Avec déjà {loyalty.purchaseCount}{" "}
              commandes passées chez l&rsquo;atelier, tu peux me demander gratuitement un print, un
              dessin ou une commission — au choix en digital, traditionnel, en tirage envoyé par la
              poste ou en fichier numérique par e-mail. Passe cette commande, puis écris-moi pour en
              profiter.
            </div>
          )}
          <input
            ...
            placeholder={
              shipping === "mondialrelay"
                ? "Point relais choisi (nom et adresse)"
                : "Numéro et rue"
            }
            value={form.address}
      ...
      <section className="mb-9">
        ...
        <div className="flex flex-col gap-2.5">
          {(Object.entries(SHIPPING_OPTIONS) as [ShippingKey, typeof SHIPPING_OPTIONS[ShippingKey]][]).map(
            ([key, opt]) => (
              <label
                key={key}
                className={`flex items-center gap-3 px-4 py-3.5 border cursor-pointer transition-colors ${
                  shipping === key ? "border-[#181614] bg-[#F2F0EA]" : "border-[#DEDAD1]"
                }`}
              >
                <input
                  type="radio"
      ...
      <section className="mb-9">
        ...
        <form onSubmit={handleStripePay}>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full bg-[#181614] text-white py-4 text-sm uppercase tracking-wide font-semibold hover:bg-[#B23A24] transition-colors disabled:opacity-50"
            >
      ...
      {showCancelModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-6">
          <div className="bg-[#FAFAF8] max-w-sm w-full p-6">
            ...
            {isLoggedIn ? (
              <>
                <p className="text-sm text-[#3A3631] mb-5">
                  Ton panier est sauvegardé dans ton compte — tu pourras reprendre ta commande plus tard depuis ton espace client.
                </p>
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 py-3 bg-[#181614] text-white text-sm uppercase tracking-wide font-semibold hover:bg-[#B23A24] transition-colors"
                  >
                    Quitter et sauvegarder
                  </button>
                  <button
                    onClick={() => setShowCancelModal(false)}
                    className="flex-1 py-3 border border-[#DEDAD1] text-sm text-[#3A3631] hover:border-[#181614] transition-colors"
                  >
              ...
              <>
                <p className="text-sm text-[#3A3631] mb-2">
                  Tu n&rsquo;as pas de compte — si tu quittes maintenant, ton panier sera perdu.
                </p>
                <p className="text-sm text-[#8C8780] mb-5">
                  Crée un compte gratuit pour sauvegarder ta commande et y revenir plus tard.
                </p>
                <div className="flex flex-col gap-2">
                  <button
                    onClick={() => {
                      setShowCancelModal(false);
                      router.push("/compte/inscription?redirect=/commande");
                    }}
                    className="w-full py-3 bg-[#181614] text-white text-sm uppercase tracking-wide font-semibold hover:bg-[#B23A24] transition-colors"
                  >
                    Créer un compte et sauvegarder
                  </button>
                  <button
                    onClick={() => setShowCancelModal(false)}
                    className="w-full py-3 border border-[#DEDAD1] text-sm text-[#3A3631] hover:border-[#181614] transition-colors"
                  >
                    Continuer ma commande
                  </button>
                  <button
                    ...
                    className="w-full py-2 text-xs text-[#8C8780] hover:text-[#B23A24] transition-colors"
                  >
                    Non merci, vider le panier et quitter
                  </button>
                </div>
              </>
            )}
