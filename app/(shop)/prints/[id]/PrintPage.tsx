...
export default function PrintPage({ product, allProducts }: { product: Product | null; allProducts: Product[] }) {
  ...
  function priceFrom(p: Product) {
    ...
    for (const k of ["print","printEmail","originalPrint","originalPrintEmail","canvas"] as const) {
  ...
  const similar = allProducts.filter(p=>p.id!==product.id&&p.active)
    .sort((a,b)=>{
      let s=0;
      if(a.medium===product.medium)s+=3; if(a.type===product.type)s+=2; s+=a.editionSold||0;
      let t=0;
      if(b.medium===product.medium)t+=3; if(b.type===product.type)t+=2; t+=b.editionSold||0;
      return t-s;
    }).slice(0,3);
        ...
        <div className="fixed inset-0 z-[70] bg-[#181614]/70 flex items-center justify-center p-6" onClick={()=>setWallOpen(false)}>
          <div className="max-w-lg w-full" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#FAFAF8] px-5 py-4 flex items-center justify-between gap-4">
              ...
              <button onClick={()=>setWallOpen(false)} className="text-xs uppercase tracking-wide font-medium text-[#181614] hover:text-[#B23A24] flex-shrink-0">Fermer</button>
            </div>
          </div>
        </div>
      )}
