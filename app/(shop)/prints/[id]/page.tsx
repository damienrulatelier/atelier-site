...
export default function PrintDetailPage() {
  ...
  // Calcule une couleur moyenne (assombrie) de l'image vedette du produit,
  // pour que le cadre de l'aperçu mural s'adapte à chaque œuvre plutôt
  // que d'avoir une seule teinte fixe pour tous les produits.
  useEffect(() => {
    ...
    img.onload = () => {
      try {
        ...
        const sampleSize = 40; // on réduit l'image pour calculer vite
        ...
        // On assombrit légèrement la moyenne pour garder un effet "cadre",
        // jamais une couleur trop claire ou trop vive.
        const darken = 0.55;
  ...
  // +3 points si même médium
  // +2 points si même type
  // +1 point par vente (editionSold) — les plus vendus remontent
  // Le produit courant est exclu, les inactifs aussi
  ...
  return (
    <main>
      <div className="max-w-6xl mx-auto px-6 md:px-8 py-8">
        <button
          onClick={() => router.back()}
          className="text-sm text-[#8C8780] hover:text-[#181614] mb-6 inline-flex items-center gap-1.5"
        >
          ← Retour
        </button>

        <div className="grid md:grid-cols-2 gap-10 md:gap-14">
          {/* IMAGE avec onglets si original + print */}
          <div>
            {(() => {
                ...
                <>
                  ...
                  {hasTabs && (
                    <div className="flex gap-0 mb-3 border border-[#DEDAD1]">
                      <button
                        type="button"
                        onClick={() => { setPhotoTab("original"); setActiveImage(0); }}
                      ...
                      <button
                        type="button"
                        onClick={() => { setPhotoTab("print"); setActiveImage(0); }}
                  ...
                  <button
                    onClick={() => activePhotos.length > 0 && setZoomOpen(true)}
                    className="w-full bg-[#F2F0EA] border border-[#DEDAD1] relative overflow-hidden cursor-zoom-in block"
                  >
                    ...
                    {activePhotos[activeImage] ? (
                      ...
                      <img
                        src={activePhotos[activeImage]}
                        alt={product.title}
                        className="w-full h-auto block"
                      />
                    ) : (
                      <div className="w-full aspect-square flex items-center justify-center text-[#8C8780] text-sm">
                        Pas de photo
                      </div>
                    )}
                  </button>
                    ...
                    <div className="flex gap-2 mt-3">
                      {activePhotos.map((img, i) => (
                        <button
                          ...
                          className={`w-16 h-16 border overflow-hidden flex-shrink-0 ${i === activeImage ? "border-[#181614]" : "border-[#DEDAD1]"}`}
                        >
                          {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            {product.wallPreviewEnabled && product.images.length > 0 && (
              <button
                ...
                className="mt-4 text-xs uppercase tracking-wide font-medium text-[#3A3631] border border-[#DEDAD1] px-4 py-2.5 hover:border-[#181614] transition-colors inline-flex items-center gap-2"
              >
          ...
          {/* INFOS */}
          <div>
            <h1 className="font-serif text-[32px] md:text-[38px] leading-tight text-[#181614] mb-2">
              {product.title}
            </h1>
            ...
            {product.type === "original" && (
              <p className="text-sm text-[#B23A24] font-medium italic mb-6">
                Pièce unique — reproductions et autres formats disponibles ci-dessous.
              </p>
            )}
            ...
            {(() => {
              ...
              return soldOut ? (
                <div className="w-full md:w-auto px-8 py-4 text-sm uppercase tracking-wide font-semibold bg-[#3A3631] text-white text-center cursor-not-allowed opacity-70">
                  Édition épuisée
                </div>
              ) : (
                <button
                  onClick={() => setAddOpen(true)}
                  className="w-full md:w-auto px-8 py-4 text-sm uppercase tracking-wide font-semibold bg-[#181614] text-white hover:bg-[#B23A24] transition-colors"
                >
            ...
            {product.allowDedication && (
              <p className="text-xs text-[#8C8780] mt-4">
                ✎ Une dédicace personnalisée pourra être ajoutée au moment de la commande.
              </p>
            )}

            {/* Lien croisé vers l'original lié si disponible */}
            {product.linkedProductId && (() => {
              ...
              const prices = d ? [
                d.original?.enabled ? d.original.price : null,
                d.originalEmail?.enabled ? d.originalEmail.price : null,
              ].filter((v): v is number => v !== null && v > 0) : [];
              const comboMin = prices.length ? Math.min(...prices) : 0;
              return (
                <div className="mt-6 p-4 border border-[#B23A24] bg-[#FAF8F5]">
                  <p className="text-xs uppercase tracking-wide font-semibold text-[#B23A24] mb-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              {similar.map((p) => (
                <Link
                  key={p.id}
                  href={`/prints/${p.id}`}
                  className="bg-[#FAFAF8] border border-[#DEDAD1] flex flex-col group"
                >
                  <div className="bg-[#F2F0EA] relative overflow-hidden">
                    {p.type === "original" && (
                      <span className="absolute top-3 left-3 font-mono text-[9px] px-1.5 py-0.5 z-10 border bg-[#B23A24] text-white border-[#B23A24]">
                        ORIGINAL
                      </span>
                    )}
                    {p.images[0] ? (
                      ...
                      <div className="aspect-square w-full flex items-center justify-center text-[#8C8780] text-xs">
                        Pas de photo
                      </div>
                    )}
                  </div>
                  <div className="p-4 flex flex-col gap-1">
                    <h3 className="font-serif text-[15px] text-[#181614] group-hover:text-[#B23A24] transition-colors truncate">
                      {p.title}
                    </h3>
                    <span className="font-mono text-[13px] text-[#3A3631]">
                      dès {fmtPrice(priceFrom(p))}
                    </span>
                  </div>
                </Link>
              ))}
      ...
      {wallPreviewOpen && (
        <div
          className="fixed inset-0 z-[70] bg-[#181614]/70 flex items-center justify-center p-6"
          onClick={() => setWallPreviewOpen(false)}
        >
          <div className="max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
            <div
              ...
              style={{
                background:
                  "linear-gradient(165deg, #EDEAE2 0%, #E4E0D5 55%, #DCD7C9 100%)",
              }}
            >
              {/* Ombre douce au sol pour ancrer la scène */}
              <div className="absolute bottom-0 inset-x-0 h-10 bg-gradient-to-t from-[#181614]/10 to-transparent" />

              {/* Cadre simulé — proportions adaptées à l'œuvre */}
              <div
                className="relative w-[46%] p-2 shadow-[0_10px_30px_rgba(24,22,20,0.35)]"
            ...
            <div className="bg-[#FAFAF8] px-5 py-4 flex items-center justify-between gap-4">
              <p className="text-xs text-[#8C8780]">
                Aperçu indicatif. Le cadre n&rsquo;est pas inclus à l&rsquo;achat — seul le tirage
                est vendu.
              </p>
              <button
                onClick={() => setWallPreviewOpen(false)}
                className="text-xs uppercase tracking-wide font-medium text-[#181614] hover:text-[#B23A24] flex-shrink-0"
              >
                Fermer
              </button>
            </div>
