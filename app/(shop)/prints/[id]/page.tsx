  ...
  useEffect(() => {
    fetch(`${window.location.origin}/api/products`)
      ...
      .then(data => {
        ...
        if (found) {
          setSimilar(
            all.filter(p => p.id !== id && p.active)
              .map(p => ({ p, s: (p.medium === found.medium ? 3 : 0) + (p.type === found.type ? 2 : 0) + (p.editionSold || 0) }))
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-8 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-10 md:gap-14">
          <div>
            {hasTabs && (
              <div className="flex mb-3 border border-[#DEDAD1]">
                <button onClick={() => { setTab("original"); setActiveImg(0); }} className={`flex-1 py-2 text-xs uppercase tracking-wide font-semibold transition-colors ${tab === "original" ? "bg-[#B23A24] text-white" : "text-[#3A3631] hover:bg-[#F2F0EA]"}`}>✦ Œuvre originale</button>
                <button onClick={() => { setTab("print"); setActiveImg(0); }} className={`flex-1 py-2 text-xs uppercase tracking-wide font-semibold transition-colors ${tab === "print" ? "bg-[#181614] text-white" : "text-[#3A3631] hover:bg-[#F2F0EA]"}`}>Print</button>
              </div>
            )}
            <div onClick={() => photos.length > 0 && setZoomOpen(true)} className="w-full bg-[#F2F0EA] border border-[#DEDAD1] overflow-hidden cursor-zoom-in">
              {photos[activeImg]
          ...
          <div>
            ...
            {product.type === "print" && product.editionTotal > 0 && <div className="mb-4"><EditionNumberBadge productId={product.id} editionTotal={product.editionTotal} /></div>}
            {soldOut
