...
export default function PrintDetailPage() {
  ...
  useEffect(() => { setFromOrig(new URLSearchParams(window.location.search).get("from")==="originals"); }, []);
  useEffect(() => {
  ...
  const prints=(product.imagesPrint||[]).length>0?product.imagesPrint!:product.images;
  const hasTabs=hasOrig&&prints.length>0&&(isDrop||(product.imagesPrint||[]).length>0);
  const photos=hasTabs?(tab==="original"?product.imagesOriginal!:prints):fromOrig&&hasOrig?product.imagesOriginal!:product.images;
  function priceFrom(p: Product) {
    if(!p.delivery) return p.price||0;
    const prices: number[]=[]; const d=p.delivery;
  ...
  const similar=allProducts.filter(p=>p.id!==product.id&&p.active)
  ...
  return (
    <main>
      <div className="max-w-6xl mx-auto px-6 md:px-8 py-8">
        <button onClick={()=>router.back()} className="text-sm text-[#8C8780] hover:text-[#181614] mb-6 inline-flex items-center gap-1.5 cursor-pointer">← Retour</button>
        <div className="grid md:grid-cols-2 gap-10 md:gap-14">
          <div>
            {hasTabs&&(
              <div className="flex mb-3 border border-[#DEDAD1]">
                <button type="button" onClick={()=>{setTab("original");setActiveImg(0);}} className={`flex-1 py-2 text-xs uppercase tracking-wide font-semibold transition-colors cursor-pointer ${tab==="original"?"bg-[#B23A24] text-white":"text-[#3A3631] hover:bg-[#F2F0EA]"}`}>✦ Œuvre originale</button>
                <button type="button" onClick={()=>{setTab("print");setActiveImg(0);}} className={`flex-1 py-2 text-xs uppercase tracking-wide font-semibold transition-colors cursor-pointer ${tab==="print"?"bg-[#181614] text-white":"text-[#3A3631] hover:bg-[#F2F0EA]"}`}>Print</button>
              </div>
            )}
            <button onClick={()=>photos.length>0&&setZoomOpen(true)} className="w-full bg-[#F2F0EA] border border-[#DEDAD1] overflow-hidden cursor-zoom-in block">
              {photos[activeImg]?(<img src={optimizeImage(photos[activeImg],1200)} alt={product.title} className="w-full h-auto block"/>):<div className="aspect-square flex items-center justify-center text-[#8C8780] text-sm">Pas de photo</div>}
            </button>
            {photos.length>1&&(
          ...
          <div>
            ...
            {soldOut?(
              ...
              <button onClick={()=>setAddOpen(true)} className="w-full md:w-auto px-8 py-4 text-sm uppercase tracking-wide font-semibold bg-[#181614] text-white hover:bg-[#B23A24] transition-colors cursor-pointer">
