...
export default function AddToCartModal({
  ...
  // Cases cochées par le client — une par format + email + original
  ...
  // Formats disponibles dans n'importe quel mode incluant un print
  ...
  // Formats toile disponibles
  const availableCanvasSizes = ALL_SIZES.filter((s) =>
    d.canvas?.sizes[s]?.enabled
  );

  function priceForCanvas(s: SizeKey): number {
    return d.canvas?.sizes[s]?.price || 0;
  }

  const [checkedCanvasSizes, setCheckedCanvasSizes] = useState<Partial<Record<SizeKey, number>>>({});

  // Prix d'un format print
  function priceForSize(s: SizeKey): number {
    return (
      ...
      d.originalPrintEmail?.sizes[s]?.enabled ? d.originalPrintEmail.sizes[s].price :
      0
    );
  }
  ...
  const hasPhysical = checkedOriginal || hasPrint || hasCanvas;
  ...
  // Remise sur le fichier numérique si combiné avec un achat physique
  const emailBasePrice = d.email?.price || 0;
  const emailDiscount = checkedEmail
    ? checkedOriginal ? emailBasePrice
    : hasPrint       ? Math.min(3, emailBasePrice)
    : 0
    : 0;
  ...
  function confirm() {
    ...
    Object.entries(checkedSizes).forEach(([s, qty]) => {
      if (((qty as number) || 0) > 0) lines.push(`${qty}× Print ${s}`);
    });
    Object.entries(checkedCanvasSizes).forEach(([s, qty]) => {
      if (((qty as number) || 0) > 0) lines.push(`${qty}× Toile ${s}`);
    });
    if (checkedEmail) lines.push("Fichier numérique");
    if (checkedOriginal) lines.push("Œuvre originale");
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#FAFAF8] w-full md:max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                      ...
                      {checked && (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="w-7 h-7 border border-[#DEDAD1] flex items-center justify-center hover:border-[#181614] text-sm disabled:opacity-40"
                          >+</button>
                        </div>
                      )}
            ...
            <label className={`${rowCls} ${checkedOriginal ? "border-[#B23A24] bg-[#FAF0EE]" : "border-[#B23A24]/30"}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-[#8C8780]">
                    expédiée — une seule au monde
                    {product.size ? ` · ${product.size}` : ""}
                  </div>
                </div>
              </div>
              <span className="font-mono text-sm font-bold text-[#B23A24]">
                {fmt(originalPrice)}
              </span>
            </label>
          </div>
        ...
        <div className="mb-4">
          ...
          <textarea
            placeholder="Une précision, une demande spéciale… je ferai en fonction."
        ...
        <div className="pt-4 border-t border-[#DEDAD1]">
          ...
          <div className="flex items-center justify-between gap-4 mt-2">
            ...
            <button
              ...
              className="flex-1 bg-[#181614] text-white py-3.5 text-sm uppercase tracking-wide font-semibold hover:bg-[#B23A24] transition-colors disabled:opacity-40"
            >
              Ajouter au panier
            </button>
          </div>
          {nothingSelected && (
            <p className="text-[11px] text-[#8C8780] mt-2 text-center">Sélectionne au moins une option</p>
          )}
        </div>
