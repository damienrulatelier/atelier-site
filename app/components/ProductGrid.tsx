    ...
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5 xl:gap-6">
        {products.map((p) => {
          ...
          const isNew = !p.temporaryUntil && p.type !== "drop" && (p.createdAt
            ...
            >
              ...
              <div className="p-6 flex flex-col gap-1.5 flex-1">
                ...
                <div className="flex items-center gap-2 flex-wrap mt-0.5">
                  ...
                  {p.temporaryUntil && new Date(p.temporaryUntil).getTime() > Date.now() && <CountdownLive until={p.temporaryUntil} />}
                  {p.retireAt && new Date(p.retireAt).getTime() > Date.now() && <CountdownLastChance until={p.retireAt} />}
                </div>
                {remaining !== null && (
