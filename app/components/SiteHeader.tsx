    ...
    <header className="sticky top-0 z-30 bg-[#FAFAF8]/95 backdrop-blur-sm border-b border-[#DEDAD1]">
      <div className="max-w-6xl mx-auto px-6 md:px-8 h-[84px] flex items-center justify-between">
        ...
        <nav className="hidden md:flex gap-9 text-xs uppercase tracking-wide font-medium text-[#3A3631]">
          <Link href="/nouveautes" className="text-[#B23A24] font-bold hover:text-[#181614] transition-colors">✦ Nouveautés</Link>
          <Link href="/#originals" className="hover:text-[#181614]">Originaux</Link>
          <Link href="/#prints" className="hover:text-[#181614]">Prints</Link>
          {hasActiveDrop && (
          ...
          <Link href="/#bd" className="hover:text-[#181614]">Bande dessinée</Link>
          <Link href="/commissions" onClick={handleCommissionsClick} className="hover:text-[#181614]">Commissions</Link>
        </nav>
        <div className="flex items-center gap-3">
          ...
          <CartButton />
          <button onClick={() => setMenuOpen((v) => !v)} aria-label={menuOpen ? "Fermer le menu" : "Ouvrir le menu"} aria-expanded={menuOpen} className="md:hidden flex flex-col gap-1.5 p-2 -mr-2">
      ...
      {menuOpen && (
        <nav className="md:hidden border-t border-[#DEDAD1] bg-[#FAFAF8] px-6 py-5 flex flex-col gap-4 text-xs uppercase tracking-wide font-medium text-[#3A3631]">
          <Link href="/nouveautes" onClick={() => setMenuOpen(false)} className="text-[#B23A24] font-bold">✦ Nouveautés</Link>
          <Link href="/#originals" onClick={() => setMenuOpen(false)} className="hover:text-[#181614]">Originaux</Link>
          <Link href="/#prints" onClick={() => setMenuOpen(false)} className="hover:text-[#181614]">Prints</Link>
          {hasActiveDrop && (
          ...
          <Link href="/#bd" onClick={() => setMenuOpen(false)} className="hover:text-[#181614]">Bande dessinée</Link>
          <Link href="/commissions" onClick={handleCommissionsClick} className="hover:text-[#181614]">Commissions</Link>
        </nav>
      )}
    </header>
